...
module.exports = function configure(config) {
  ...
  const labels = {
    'faas_name': config.name || 'anonymous',
    'faas_id': config.id || process.env.FAAS_ID || `${process.env.POD_NAME}-${randomUUID()}`,
    'faas_instance': config.instance || process.env.POD_NAME,
    'faas_runtime': config.runtime || 'Node.js',
  };
