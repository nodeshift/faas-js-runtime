...
function installDependenciesIfExist(functionPath) {
  let _path = functionPath;
  if (path.extname(_path) !== '') {
    _path = path.dirname(_path);
  }
  _path = path.join(fixtureDir, _path);
  if (existsSync(path.join(_path, 'package.json'))) {
    // eslint-disable-next-line
    console.log(`Installing dependencies for ${_path}`);
    execSync('npm install --omit=dev', { cwd: _path });
  }
}
...
test('Function accepts destructured parameters', (t) => {
  start(function ({ lunch }) {
    return { message: `Yay ${lunch}` };
  }).then((server) => {
    t.plan(2);
    request(server)
      .get('/?lunch=tacos')
...
test('Reads a func.yaml file for logLevel', (t) => {
  ...
  start(func, { config: funcPath }).then((server) => {
    request(server)
      .get('/')
      .expect(204)
      .end((err, _) => {
